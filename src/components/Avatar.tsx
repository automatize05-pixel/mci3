...
const UserAvatar = ({ userId, src, name, username, isChef, isVerified, size = "md", linked = true }: AvatarProps) => {
  ...
  const content = (
    <div className={`relative flex-shrink-0 ${linked && userId ? "cursor-pointer" : ""}`}>
      ...
      {(isChef || isVerified) && (
        <div className={`absolute ${badgeSizes[size]} ${isVerified ? "text-[#1d9bf0]" : "bg-chef"} rounded-full flex items-center justify-center ring-2 ring-card z-10 shadow-sm overflow-hidden bg-card`}>
          {isVerified ? (
            <svg viewBox="0 0 24 24" className="w-[70%] h-[70%] fill-current flex-shrink-0">
