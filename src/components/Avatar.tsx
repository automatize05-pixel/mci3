...
const UserAvatar = ({ userId, src, name, username, isChef, isVerified, size = "md", linked = true }: AvatarProps) => {
  ...
  const content = (
    <div className={`relative flex-shrink-0 ${linked && userId ? "cursor-pointer" : ""}`}>
      ...
      {(isChef || isVerified) && (
        <div className={`absolute ${badgeSizes[size]} ${isVerified ? "text-[#1d9bf0] bg-background" : "bg-secondary"} rounded-full flex items-center justify-center ring-2 ring-background z-10 shadow-sm border-0`}>
          {isVerified ? (
